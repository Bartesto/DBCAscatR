...
is the same as shown in the elbow graph. The probability of misassignment is also 
calculated which can be converted into the percentage of error. For example, 
graph a) the probability of assigning samples to a wrong group is 0.000006%. The 
minimum suggested percentage is 0.5%. Overlap of the upper 0.995 percentile (red 
dashed line) and lower 0.005 percentile (blue dashed line) suggested the 
possibility of misassigning samples to between and within group distributions. 
The location of the overlap suggests which distribution the samples will likely 
to misassign to. For example, if the overlap falls in the between group 
distribution (blue), the samples from the within group distribution (red) are 
likely to be misassigned to the between group distribution (blue).

...
## Structure format
The following code exports the group majorities data to csv.
...
## Summary tables
Metadata comes in all shapes and sizes and may largely depend on who collects the 
data and what it is collected in as to what format it is in. For the summary 
tables function to work there are two things that the user needs to ensure:
...
* Yellow arrows - shows sites or locations depending what layer you are showing. 
Blue marker is a single entity. Green circle with number indicates multiple 
records at this location. Zooming in on a green circle will "explode" the 
entities so all can be interrogated (mouse over) for information.
